#!/usr/bin/env python3
"""
Step 6 (interactive) — manually digitize field elements from a reference
field image, save them keyed by year for 07_field_calibrate.py to load as
built-in draggable elements (so a fresh calibration starts with them
pre-placed on the field side instead of needing to be added by hand).
...
Workflow:
  ...
  4. "Save as year" writes every finished element's points (converted to
     ...
     data/frc/field_elements.json, keyed by year. 07_field_calibrate.py
     reads this file and requires a year to have at least one saved entry.
...
Usage:
  python 06_field_elements_editor.py --year 2026
  python 06_field_elements_editor.py --year 2026 --image field.png
"""
import argparse
import json
import pathlib
...
class Editor:
    ...
    def _load_image(self, path):
        ...
        if img is None:
            ...
            self.status.config(text=msg) if hasattr(self, "status") else print(f"[editor] {msg}")
            return
        self.img_bgr = img
        self.img_path = path
    ...
    def _save_draft(self):
        ...
        p.write_text(json.dumps(self._draft_data(), indent=2))
    ...
    def save(self):
        ...
        if not out:
            self.status.config(text="nothing to save (no finished elements with >=3 points)")
            return
        data = load_all()
