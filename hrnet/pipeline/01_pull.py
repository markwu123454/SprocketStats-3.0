...
# Anchor relative config paths to the module root (hrnet/) regardless of cwd.
...
# "Unknown" = a visible robot whose alliance the annotator could not determine.
# Mapped here (rather than dropped) so the annotation survives the pull; whether
# it is USABLE depends on the training mode, which is 02_train.py's call, not
# this script's. Optional so an older project without the third label still works.
if LS.get("label_unknown"):
    LABEL_TO_CLS[LS["label_unknown"]] = "unknown"
# "Climbing" = robot present but off the ground; not a ground-plane point.
# Pulled through so it survives, then filtered out of heatmap targets by
# model.py. Keeping it in the manifest is what makes the decision reversible.
if LS.get("label_climbing"):
    LABEL_TO_CLS[LS["label_climbing"]] = "climbing"
...
def main():
    ...
    ap.add_argument("--project", type=int, default=None,
                    help="Label Studio project id (default: config "
                         "label_studio.project_id).")
    ap.add_argument("--out", default=None,
                    help="manifest filename under paths.export_dir "
                         "(default: manifest.jsonl). Use a per-project name "
                         "when pulling more than one project.")
    args = ap.parse_args()
    ...
    snap_path = export_snapshot(client, project_id,
                                reuse=args.reuse, use_local_cache=args.local_cache)
    tasks = json.load(open(snap_path))
    print(f"[parse] {len(tasks)} tasks in snapshot")

    s3 = make_r2()
    # NOTE: the manifest name is fixed by default, so pulling a SECOND project
    # would overwrite the first project's manifest. Pass --out to keep them
    # separate (e.g. manifest_279511.jsonl) when you intend to train on both.
    manifest_path = EXPORT_DIR / (args.out or "manifest.jsonl")
    kept = 0
