#!/usr/bin/env python3
"""
One-off tool (not part of the numbered pipeline) -- Calibration, part 2:
fuse the raw digit-glyph crops harvested by calibrate.py (alongside this
file in tools/) into one higher-quality template per (kind, digit). Pure
image processing over already-saved crops, no video/OCR -- deliberately
separate from collection, which is video/OCR-bound. See calibrate.py's
...
1. Normalize every crop to grayscale, and for `kind == "clock"` INVERT it
   (clock digits are dark-on-white -- see calibrate.py's polarity
   ...
   calibrate.py), so a directory-name check can no longer tell them
   apart -- calibrate.py now normalizes polarity itself at harvest time
...
5. Take the per-pixel MEDIAN (not mean) across all aligned instances --
   ...
   that survived calibrate.py's count-agreement check by coincidence. This
   is the final template.

Output
------
data/digit_templates/<kind>/<digit>.png -- the merged template.
data/digit_templates/<kind>/<digit>_compare.png -- only with --compare:
SINGLE raw instance (plain cubic upsample, no alignment/averaging) vs
MERGED template, side by side, at the same scale -- so the improvement (or
lack of one) is something to actually look at, not just infer from code.
Same "a verification image the user looks at catches things a numeric
check doesn't" lesson as detection's own validation (see broadcast's
project memory).
...
- Badge templates from match6's blue_badges[0] are absent from that bucket
  ...
  calibrate.py's run log) -- the "badge" bucket's other 6 matches' worth
...
-----
  ...
  python tools/build_templates.py --compare  # also write debug compare strips
"""
...
def main():
    ...
    ap.add_argument("--compare", action="store_true",
                    help="also write a {digit}_compare.png single-vs-merged debug strip per template")
    args = ap.parse_args()

    if not INSTANCES_DIR.exists():
        raise SystemExit(f"[error] {INSTANCES_DIR} not found -- run calibrate.py --save first")
